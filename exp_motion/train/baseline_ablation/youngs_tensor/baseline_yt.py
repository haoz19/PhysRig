...
改回youngs tensor进行debug

1.1 采用smooth loss，没有用

1.2 检查是不是cuboid导致梯度混乱，设定距离cuboid x的点gt为定值，并且设置mask，不更新距离为r内的点

1.3 全局youngs为一个值，即只学[1]的tensor
...
class Trainer:
    ...
    def init_trainable_params(self,):
        ...
        #     torch.ones(self.num_particles, device=self.device, dtype=torch.float32) * args.youngs,
        #     requires_grad=True
        # )
        
        youngs_modulus = nn.Parameter(
            torch.tensor(args.youngs, device=self.device, dtype=torch.float32), 
            requires_grad=True
        )
    ...
    def setup_simulation(self, dataset_dir, grid_size=80):
        ...
        self.neighbor_indices = get_knn_neighbors(sim_xyzs, k=6)
        ...
        self.cuboid_points_indices = get_points_in_cuboid_sphere(sim_xyzs, self.cuboid_point, self.cuboid_size)
        # print(f"每个 cuboid 包裹的球形范围内找到的点数:")
        # for idx, indices in enumerate(self.cuboid_points_indices):
        #     print(f"Cuboid {idx}: {indices.numel()} 个点")
            
        self.cuboid_points_indices = torch.unique(torch.cat(self.cuboid_points_indices))
        # print(f"cuboid 包裹的球形范围内找到的点数: {sim_xyzs[self.cuboid_points_indices].shape}")
...
#         pdb.set_trace()
        ...
        youngs_modulus = self.E_nu_list[0].repeat(self.num_particles)
    ...
    def get_material_params(self, device):
        ...
        # youngs_modulus[self.cuboid_points_indices] = 6000.0
...
#                     continue
#                 # 取出该部分的预测点和 GT 点
#                 pred_part = predicted_points[indices]  # [Np, 3]
#                 target_part = target_points[indices]     # [Np, 3]
...
#                 local_loss = 0.0
            ...
            # smooth loss
#             smooth_loss = compute_smoothness_loss(youngs_modulus, self.neighbor_indices, particle_pos)
            
#             sm_loss_weight = 1e-2
            
#             loss = loss + smooth_loss * sm_loss_weight
...
#         high_90, low_40 = get_gradient_percentiles(self.E_nu_list[0], percentile_high=90, percentile_low=40)
#         print(f"90% percentile: {high_90:.5e}, 40% percentile: {low_40:.5e}")
                
#         for param in self.trainable_params:
#             smooth_clip_gradients(param, min_val=low_20, max_val=high_80)
            
        # check_gradients(
        #     ("After self.E_nu_list[0]", self.E_nu_list[0])  # 位置参数
        # )
