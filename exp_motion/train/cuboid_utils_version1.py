...
def cuboid_finding(vertices, grid_dx, vertices_assignment=None, mesh=None):
    ...
    if (vertices_assignment is None or len(vertices_assignment) == 0) and mesh is None:
        ...
        # 特殊索引使用0.6倍，其他使用0.4倍
        special_indices = {6, 13, 16, 31}
        
        for i in range(vertices_original.shape[0]):
            center = vertices_original[i]
            cuboid_centers.append(center)
            # 找到点i到其他所有点的距离，排除自己
            dists_to_others = dists[i].copy()
            dists_to_others[i] = np.inf  # 排除自己
            min_dist_to_nearest = np.min(dists_to_others)  # 到最近点的距离
            # 根据索引选择不同的倍数
            if i in special_indices:
                radius = min_dist_to_nearest * 0.6  # 特殊索引使用0.6倍
            else:
                radius = min_dist_to_nearest * 0.4  # 其他使用0.4倍
            # 将标量radius转换为3维向量
...
def assign_cuboid_velocity(cuboid_centers, cuboid_sizes, cuboid_types, total_frames, gt_meshes, vertices, delta_time, device, output_dir=None, num_intermediate_frames: int = 0):
    ...
    for cuboid_idx, (cuboid_center, cuboid_size, cuboid_type) in enumerate(zip(cuboid_centers, cuboid_sizes, cuboid_types)):
        if cuboid_type != 'point_sphere':
            raise ValueError(f"未知 Cuboid 类型: {cuboid_type}")
        cuboid_center_tensor = torch.tensor(cuboid_center, dtype=torch.float32, device=device)
        distances = torch.norm(vertices_tensor - cuboid_center_tensor, dim=1)
