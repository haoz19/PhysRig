...
class Trainer:
    def __init__(self, args):
        ...
        gtmesh_dir = os.path.join(dataset_dir, "7e3_3e3gt")
        ...
        self.trainable_params1 = self.E_nu_list
        
        self.trainable_params2 = self.cuboid_velocity
...
#         )
        ...
        self.velocity_scheduler = get_linear_schedule_with_warmup(
            ...
            num_warmup_steps=args.warmup_step,
            num_training_steps=(args.train_iters - args.iter_material),
        )
    ...
    def setup_simulation(self, dataset_dir, grid_size=80):
        ...
        # Haolan：创建gt_mask
        gt_mesh_initial = os.path.join(dataset_dir, "./7e3_3e3gt/gt_0.ply")
        ...
        self.normalized_weights = get_youngs_weights(self.particle_init_position, self.sample_indices, self.output_dir)
        
        youngs_modulus = (self.normalized_weights * self.E_nu_list[0].clone().unsqueeze(0)).sum(dim=1)
    ...
    def get_material_params(self, device):
        ...
        sample_youngs = self.E_nu_list[0]  # shape: (self.sample_particles,)
    ...
    def train_one_step(self):
        ...
        torch.nn.utils.clip_grad_norm_(
            self.trainable_params1,
            self.max_grad_norm,
            error_if_nonfinite=False,
        )  # error if nonfinite is false
        
        torch.nn.utils.clip_grad_norm_(
            self.trainable_params2,
            self.max_grad_norm,
            error_if_nonfinite=False,
        )  # error if nonfinite is false
        ...
        # 开始一起更新速度
        if self.step >= self.iter_material:
        ...
        if log_loss_dict['Frame_loss'] <= 0.1:
        ...
        if (young_min_val < 0) or any([
            is_invalid(nu_val),
            is_invalid(young_min_val),
            is_invalid(young_mean_val),
            is_invalid(young_max_val)
        ]):
...
def parse_args():
    ...
    parser.add_argument("--lr_weights", type=float, default=1e-5)
...
if __name__ == "__main__":
    ...
    print("Training material and velocity in Cuboid Case")
