...
class Trainer:
    def __init__(self, args):
        ...
        # 用于计算cuboid velocity
        gtmesh_dir_og = os.path.join(dataset_dir, "gt")
...
#         theta_value = np.log(args.youngs)
        
#         theta = nn.Parameter(
#             torch.ones(self.num_particles, device=self.device, dtype=torch.float32) * theta_value,
#             requires_grad=True
#         )
        ...
        # trainable_params = [theta, poisson_ratio]
    ...
    def setup_simulation(self, dataset_dir, grid_size=80):
        ...
        bbox = calculate_bounding_box(self.gt_meshes_og) # 这里要用最初gt的size来确保grid大小一样
...
#         pdb.set_trace()
    ...
    def train_one_step(self):
        ...
        for start_time_idx in range(0, window_size, temporal_stride):
            ...
            #     points_list = [sim_points_full, target_points_0]
            ...
            # points_list = [sim_points_full, target_points]
            # save_ply(points_list, self.step, self.output_dir, frame_idx=start_time_idx+1)
            
            
            chamfer_loss = chamfer_dist(predicted_points_cd, target_points_cd)
            loss = chamfer_loss * 100
            # loss = loss * (self.args.loss_decay**end_time_idx)
            
            
            # l2_loss = torch.mean((particle_pos[self.gt_mask_indices] - gt_frame) ** 2)
            # l2_weight = 1e6
            ...
            # if self.step % 10 == 0 or self.step == self.train_iters - 1:
            ...
            #     high_90, low_10 = get_gradient_percentiles(self.E_nu_list[0])
            #     print(f"90% percentile: {high_90:.5e}, 10% percentile: {low_10:.5e}")
        ...
        #     smooth_clip_gradients(param, min_val=low_10, max_val=high_90)
            
        # check_gradients(
        #     ("sim_fields", self.sim_fields),
        # )
        ...
        # with torch.no_grad():
        #     self.E_nu_list[0].data.clamp_(2000, 1e7)
