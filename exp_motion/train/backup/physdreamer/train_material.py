...
class Trainer:
    def __init__(self, args):
        ...
        self.window_size_schduler = LinearStepAnneal(
            ...
            end_state=[13], # self.num_frames - 1 
            plateau_iters=-1,
            warmup_step=20,
        )
        ...
        if args.checkpoint_path is not None:
            ...
            optim_list = [
                ...
                # {"params": self.lolo_fields.parameters(), "lr": args.lr * 1e-3, "weight_decay": 1e-4},
            ]
    ...
    def get_material_params(self, device):
        ...
        if self.args.entropy_cls > 0:
            sim_params, entropy = self.sim_fields(query_pts) # Haolan：entropy作为正则化参数，保证训练模拟过程的稳定和精度
        else:
        ...
        youngs_modulus[query_mask] += sim_params[..., 0] # Haolan：代码只用了训练更新的杨氏，但其他参数也可以训练
    ...
    def train_one_step(self):
        ...
        num_substeps = int(delta_time / substep_size) # Haolan: 预设的768
        ...
        for start_time_idx in range(0, window_size, temporal_stride):
            ...
            # Haolan：temporal_stride默认为1，window_size默认为6
            end_time_idx = min(start_time_idx + temporal_stride, window_size)

            num_step_with_grad = num_substeps * (end_time_idx - start_time_idx) # Haolan：逐帧，因此end_time_idx - start_time_idx为1
            ...
            if checkpoint_steps > 0 and checkpoint_steps < num_step_with_grad: # Haolan：checkpoint_steps默认为-1，因此在这里没有grad checkpoint，用的是MPMDifferentiableSimulationClean
                ...
                particle_pos, particle_velo, particle_F, particle_C, particle_cov = (
                    MPMDifferentiableSimulationClean.apply(
                        ...
                        device,
                        True,
                        0, # Haolan：这里定义extra_no_grad_steps为0，意味着并没有引入没梯度模拟步
                    )
                )
            ...
            # Haolan: 我们现在是GT Mesh，利用3D Loss like Chamfer Distance
    ...
    def inference(
        self,
        cam,
        velo_scaling=1.0, # Haolan：根据具体实验调整
        num_sec=1,
        nu=None,
        young_scaling=1.0,
        substep=64, # Haolan：这里是用于inference的substep，而train的时候，substep默认为768,
        youngs_modulus=None,
    ):
...
def parse_args():
    ...
    parser.add_argument("--num_frames", type=str, default=14) # Haolan: Video Clips for Model，总数是151
