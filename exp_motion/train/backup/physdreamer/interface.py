...
# Haolan:需要我们逻辑的MPMDifferentiableSimulation函数（skeleton驱动，cube）
# 先吃透forward和backward，再明确和我们逻辑的区别，让GPT协助修改，自己check
...
class MPMDifferentiableSimulationClean(autograd.Function):
    ...
    def forward(
        ...
        with cond_tape:
            ...
            for substep_local in range(num_substeps):
                next_state = prev_state.partial_clone(requires_grad=True)
                mpm_solver.p2g2p_differentiable(mpm_model, prev_state, next_state, substep_size, device=device) # Haolan：p2g2p_differentiable进行梯度计算
                next_state_list.append(next_state)
                prev_state = next_state
        
        # Haolan：ctx保存forward的信息用于backward
        ctx.mpm_solver = mpm_solver
        ctx.mpm_model = mpm_model
        ctx.next_state_list = next_state_list
        ctx.device = device
    ...
    def backward(ctx, out_pos_grad: Float[Tensor, "n 3"], out_velo_grad: Float[Tensor, "n 3"], 
        ...
        with wp.ScopedDevice(device):
            ...
            # Haolan:debug loss_wp
            with tape:
                loss_wp = torch.zeros(1, device=device)
                loss_wp = wp.from_torch(loss_wp, requires_grad=True)
                target_pos_detach = wp.clone(next_state.particle_x, device=device, requires_grad=False)
                debug_pos_buffer = wp.zeros(num_particles, dtype=wp.vec3, device=device)
                debug_diff_buffer = wp.zeros(num_particles, dtype=wp.vec3, device=device)
                debug_pos_gt_buffer = wp.zeros(num_particles, dtype=wp.vec3, device=device)
                debug_grad_buffer = wp.zeros(num_particles, dtype=wp.vec3, device=device)
                wp.launch(
                    compute_posloss_with_grad, 
                    dim=num_particles,
                    inputs=[
                ...
                # debug_pos = wp.to_torch(debug_pos_buffer)
                # debug_diff = wp.to_torch(debug_diff_buffer)
                # debug_pos_gt = wp.to_torch(debug_pos_gt_buffer)
                # debug_grad = wp.to_torch(debug_grad_buffer)
