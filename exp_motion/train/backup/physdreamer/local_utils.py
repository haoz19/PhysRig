...
class IntervalAnneal(object):
    def __init__(
        ...
        self.end_state = end_state
        self.warmup_step = warmup_step
        if end_state[0] + 1 != start_state[0]:
            self.interval_iters = self.total_iters // (int(end_state[0]) - int(start_state[0]) + 1)  # 控制 window_size 增长频率
        else:
            self.interval_iters = self.total_iters
        # self.interval_iters = 1 # For debug
    ...
    def compute_state(self, cur_iter):
        ...
        if cur_iter == 0 or cur_iter % self.interval_iters != 0:
            ret = self.start_state[0] + (cur_iter // self.interval_iters)
        elif cur_iter != 0 and cur_iter % self.interval_iters == 0:
            ret = self.start_state[0] + (cur_iter // self.interval_iters) - 1

        if ret >= self.end_state[0]:
            return self.end_state
