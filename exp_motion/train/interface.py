...
class MPMDifferentiableSimulationRig(autograd.Function):
    ...
    def forward(
        ...
        with wp_tape:
            ...
            # 将 cuboid_velocity 转为 warp 数组（批量处理）
            num_cuboids = cuboid_velocity.shape[0]
            cuboid_velocities_wp = from_torch_safe(cuboid_velocity, dtype=wp.vec3, requires_grad=True)
            
            # 将 cuboid_point 转为 warp 数组
            cuboid_positions_wp = from_torch_safe(cuboid_point, dtype=wp.vec3, requires_grad=False)
            
            # 计算 cuboid_radii（使用 size 的 x 分量作为半径）
            cuboid_radii_torch = cuboid_size[:, 0]  # 取 x 方向的大小作为半径
            cuboid_radii_wp = from_torch_safe(cuboid_radii_torch, dtype=wp.float32, requires_grad=False)
            
            # 一次性设置所有 cuboid 的速度边界条件
            mpm_solver.set_velocity_on_cuboid_diff(
                cuboid_positions=cuboid_positions_wp,
                cuboid_velocities=cuboid_velocities_wp,
                cuboid_radii=cuboid_radii_wp,
        ...
        ctx.tape = wp_tape
        ctx.cuboid_velocities_wp = cuboid_velocities_wp
    ...
    def backward(ctx, out_pos_grad: Float[Tensor, "n 3"], out_velo_grad: Float[Tensor, "n 3"], 
        ...
        # grad for E, nu, cuboid_velocity
        
        E_grad = wp.to_torch(mpm_model.E.grad).clone().to(device) 
        
        nu_grad = wp.to_torch(mpm_model.nu.grad).clone().to(device)
        
        # 直接从 cuboid_velocities_wp 获取梯度
        cuboid_velocity_grad = wp.to_torch(ctx.cuboid_velocities_wp.grad).clone().to(device)
        
        # print("cuboid_velocity_grad shape:", cuboid_velocity_grad.shape)
        # print("cuboid_velocity_grad values:", cuboid_velocity_grad.detach().cpu().numpy())
