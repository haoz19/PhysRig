...
class Trainer:
    ...
    def init_trainable_params(self,):
        ...
        # 处理mask部分的sample point
        
        mask_files = ["chest.ply", "belly.ply", "pelvis.ply"]
...
#                 # print(f"Part {p.item()}: motion={motion.item():.6f}, weight={weight.item():.6f}, part_loss={part_loss.item():.6f}")
#                 # 加权累加该区域 loss
...
#                 local_loss = 0.0
            ...
            # loss = global_loss * 100 + local_loss * 400
            loss = global_loss * 100

            # velo smooth loss
            smooth_loss = 0
            for frame_idx in range(len(self.cuboid_velocity) - 1):
                smooth_loss += torch.norm(self.cuboid_velocity[frame_idx] - self.cuboid_velocity[frame_idx + 1], p=2)
            loss = loss + smooth_loss * 0.2
            
            # print(f"CD Local Loss: {local_loss.item()}")
            print(f"CD Global Loss: {global_loss.item()}")
