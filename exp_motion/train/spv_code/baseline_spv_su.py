...
class Trainer:
    def __init__(self, args):
        ...
        gtmesh_dir = os.path.join(dataset_dir, "gt_mesh")
    ...
    def setup_simulation(self, dataset_dir, grid_size=80):
        ...
        # print(f"  x_min: {bbox['x_min']}, x_max: {bbox['x_max']}")
        # print(f"  y_min: {bbox['y_min']}, y_max: {bbox['y_max']}")
        # print(f"  z_min: {bbox['z_min']}, z_max: {bbox['z_max']}")
        ...
        self.cuboid_point = torch.tensor([(0.41, 0.45, 0.35)], dtype=torch.float32, device=device)
        self.cuboid_size = torch.tensor([(0.3, 0.1, 0.15)], dtype=torch.float32, device=device)

        # 计算新的 cuboid 大小
        new_size = self.cuboid_size / 2  # 每个 cuboid 的大小变为原来的 1/2

        # 计算偏移量：8个cuboid的相对偏移
        offset_factors = torch.tensor([
            [-1, -1, -1],  # 左下后
            [1, -1, -1],   # 右下后
            [-1, 1, -1],   # 左上后
            [1, 1, -1],    # 右上后
        ...
        # 计算新的 cuboid 中心点
        offsets = offset_factors * (new_size / 2)  # 计算相对原点的偏移量
        new_cuboid_points = self.cuboid_point + offsets  # 计算 8 个 cuboid 的中心点
        
        grid_dx_tensor = torch.tensor(1.2 * grid_dx, dtype=torch.float32, device=device)
        new_size = torch.max(new_size * 0.3, grid_dx_tensor)
        
        print("New cuboid size:", new_size)
        
        # 更新 self.cuboid_point 和 self.cuboid_size
        self.cuboid_point = new_cuboid_points  # 变成 shape (8, 3)
        self.cuboid_size = new_size.repeat(8, 1)  # 变成 shape (8, 3)
        
        # cuboid可视化
        cuboid_centers = self.cuboid_point.cpu().numpy()  
        cuboid_sizes = self.cuboid_size.cpu().numpy()  
        ...
        self.cuboid_velocity = [
            torch.nn.Parameter(torch.zeros((8, 3), dtype=torch.float32, device=device), requires_grad=True)
    ...
    def train_one_step(self):
        ...
        for start_time_idx in range(0, window_size, temporal_stride):
            ...
            if self.step == self.train_iters - 1 or (not torch.isnan(loss) and not torch.isinf(loss) and loss.item() > self.prev_loss):
                if window_size == self.num_frames:
                    if start_time_idx == 0:
        ...
        if self.prev_loss < self.loss_threshold and self.window_size < self.num_frames:
