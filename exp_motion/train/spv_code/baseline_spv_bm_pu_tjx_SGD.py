...
class Trainer:
    ...
    def setup_simulation(self, dataset_dir, grid_size=80):
        ...
        print(f"X range: {x_range:.3f}m, Y range: {y_range:.3f}m, Z range: {z_range:.3f}m")
         ...
         # Haolan：创建gt_mask
        skeleton_init_dir = args.skeleton_init_dir or os.path.join(dataset_dir, "skeleton")
        gt_mesh_initial = os.path.join(skeleton_init_dir, "gt_0.ply")
        ...
        # Save cuboid sizes for later mesh generation
        # IMPORTANT: Scale sizes to match output coordinate system
        cuboid_sizes_scaled = cuboid_sizes_np * scale.cpu().numpy()
        cuboid_sizes_path = os.path.join(self.output_dir, "cuboid_sizes.npy")
        np.save(cuboid_sizes_path, cuboid_sizes_scaled)
        print(f"Saved cuboid sizes (scaled) to {cuboid_sizes_path}")
...
def parse_args():
    ...
    default_config = os.path.join(PROJECT_ROOT, "exp_motion/train/config.yml")
    default_dataset = os.path.join(PROJECT_ROOT, "data/dragon_tjx_1/")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional skeleton directory for initial gt_0.ply",
    )
    ...
    parser.add_argument("--output_dir", type=str, default="../../output/train/dragon_tjx_1")
