...
class Trainer:
    ...
    def train_one_step(self):
        ...
        for start_time_idx in range(0, window_size, temporal_stride):
            ...
            # 验证点数是否一致（因为点是一一对应的）
            assert predicted_points.shape[0] == target_points.shape[0], \
                f"点数不一致: predicted {predicted_points.shape[0]} vs target {target_points.shape[0]}"
            
            predicted_points_cd = particle_pos.unsqueeze(0).clone()  
            target_points_cd = gt_frame.unsqueeze(0) 

            # Chamfer Distance loss             
            cd_loss = chamfer_dist(predicted_points_cd, target_points_cd)*100
            
            # MSE loss（点对点直接计算，因为点是一一对应的）
            # MSE = mean((predicted - target)^2) 对所有坐标
            mse_loss = torch.mean((predicted_points - target_points) ** 2)*10000
            
            # 打印当前帧的loss（带帧索引）
            print(f"  Frame {start_time_idx}->{end_time_idx}: CD={cd_loss.item():.6f}, MSE={mse_loss.item():.6f}")
            ...
            with torch.no_grad():
                log_loss_dict["CD_loss"].append(cd_loss.item())
                log_loss_dict["MSE_loss"].append(mse_loss.item())
        ...
        current_loss_vector = np.array(log_loss_dict["MSE_loss"])
        self.loss_array[self.step, :self.window_size] = current_loss_vector
        ...
        log_losses(
            frame_loss=log_loss_dict["MSE_loss"],
            iteration=self.step
        )

        print(f"Iteration {self.step} Summary: Avg MSE={log_loss_dict['MSE_loss']:.6f}, Avg CD={log_loss_dict['CD_loss']:.6f}")
...
def parse_args():
    ...
    parser.add_argument(
        "--dataset_dir",
        type=str,
        default="/taiga/illinois/eng/ece/n-ahuja/haozhang/tjx/PHYSDREAMER/PhysDreamer/data/dragon_tjx/",
    )
    ...
    parser.add_argument("--output_dir", type=str, default="../../output/train/dragon_tjx_mse")
