...
class MPMDifferentiableSimulationRig(autograd.Function):
    ...
    def forward(
        ...
        with wp_tape:
            ...
            # 将当前帧的 cuboid_velocity 转为 warp 变量，设置 requires_grad=True
            velocity_wp_list = []
            num_cuboids = cuboid_velocity.shape[0]
            for i in range(num_cuboids):
                vel_i_torch = cuboid_velocity[i].clone()
                vel_i_wp = from_torch_safe(vel_i_torch, dtype=wp.vec3, requires_grad=True)
                velocity_wp_list.append(vel_i_wp)
            ...
            for i in range(num_cuboids):
                ...
                mpm_solver.set_velocity_on_cuboid_diff(
                    ...
                    size=cuboid_size[i],
                    velocity_wp=velocity_wp_list[i], 
        ...
        ctx.tape = wp_tape
        ctx.velocity_wp_list = velocity_wp_list
    ...
    def backward(ctx, out_pos_grad: Float[Tensor, "n 3"], out_velo_grad: Float[Tensor, "n 3"], 
        ...
        grad_list = []
        for v_wp in ctx.velocity_wp_list:
            # v_wp.grad 是一个 warp 数组，转换为 torch tensor
            grad_v = wp.to_torch(v_wp.grad).clone().to(device)
            grad_v = grad_v.squeeze(0)
            grad_list.append(grad_v)
        # 组合成形状 [num_cuboids, 3]
        cuboid_velocity_grad = torch.stack(grad_list, dim=0)
