...
def apply_transformation(pos, relative_root_motion):
    R = relative_root_motion[:3, :3]
    T = relative_root_motion[:3, 3]
    R_tensor = torch.from_numpy(R).float().cuda()
    T_tensor = torch.from_numpy(T).float().cuda()
    # pos transformation
    pos_rotated = apply_rotation(pos, R_tensor)
    pos_transformed = pos_rotated + T_tensor
    
    # cov3D transformation?
    # cov3D_transformed = apply_cov_rotations(cov3D, R_tensor)
