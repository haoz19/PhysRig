...
def training(args, dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        loss += args.aniso_weight * loss_aniso
        loss.backward()
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:
                progress_bar.set_postfix({"Loss": f"{ema_loss_for_log:.{7}f}", "aniso": f"{loss_aniso:.{3}f}"})
...
if __name__ == "__main__":
    ...
    parser.add_argument("--aniso_thres", type=float, default = 2.0)
    parser.add_argument("--aniso_weight", type=float, default = 1e-3)
