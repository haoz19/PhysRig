...
class MPMWARPDiff(object):
    ...
    def p2g2p_differentiable(
        ...
        for k in range(len(self.pre_p2g_operations)):
            # print("are u sure?? u r entering pre_p2g_operations")
            wp.launch(
    ...
    def export_particle_R_to_torch(self, mpm_model, mpm_state, device="cuda:0"):
        with wp.ScopedTimer(
            "compute_R_from_F",
            synchronize=True,
            print=False,
    ...
    def enforce_particle_velocity_translation_fixed(
        self, mpm_state, point, size, velocity, part_id, start_time, end_time, device="cuda:0"
    ):
        # first select certain particles based on position
        # difference is judging if part_id is stored and simulated
        ...
        if part_id not in self.particle_part_chosen:
            wp.launch(
                kernel=selection_enforce_particle_velocity_translation,
                dim=self.n_particles,
                inputs=[mpm_state, velocity_modifier_params],
            ...
            self.particle_part_chosen[part_id] = velocity_modifier_params.mask
        else:
            velocity_modifier_params.mask = self.particle_part_chosen[part_id]
