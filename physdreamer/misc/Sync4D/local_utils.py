...
def get_unrelated_parts(xyzs, points, sizes):
    """
    Args:
        xyzs: [N, 3]
        points: [M, 3]
        sizes: [M, 3]
    Outs:
        freeze_mask: [N], 1 for points that are not inside cube, 0 for points that are inside cube
                    dtype=torch.int
    """
    assert points.shape[0] == sizes.shape[0]
    freeze_mask_list = torch.ones((points.shape[0], xyzs.shape[0]))
    freeze_mask_list = freeze_mask_list.bool()
    freeze_mask = torch.ones(xyzs.shape[0])
    freeze_mask = freeze_mask.bool()
    points = points.to(xyzs.device)

    # print(points, sizes)
    for i in range(xyzs.shape[0]):
        for j in range(points.shape[0]):
            offset = torch.abs(xyzs[i] - points[j])
            if (
                offset[0] < sizes[j][0]
                and offset[1] < sizes[j][1]
                and offset[2] < sizes[j][2]
            ):
                freeze_mask_list[j][i] = False
                freeze_mask[i] = False
    
    freeze_mask_list = freeze_mask_list.type(torch.bool)             
    freeze_mask = freeze_mask.type(torch.bool)
    return freeze_mask_list, freeze_mask
...
def render_gaussian_w_grid_sampling(
    cam, render_params, disp, p2v_map, frame, frame_per_motion, root_motion
):
    '''
    Dummy apply disp to all nodes in grid
    '''
    # ret_img_list = []
    gaussians = render_params.gaussians
    old_xyz = gaussians._xyz.clone()
    ...
    new_xyz = old_xyz.clone()
    # query_rotation = old_rotation
    
    background = torch.tensor([1, 1, 1], dtype=torch.float32, device="cuda")
    # new_xyz, new_rotation = interpolate_points_w_R(
    #     query_pts, query_rotation, orign_points, disp, top_k_index=8
    # )
    p2v_map = list(p2v_map)
    import sys
    for i, row in enumerate(p2v_map):
        row = row[row != 0]
        new_xyz[row] += disp[i]
    ...
    pos = gaussians._xyz.clone()
    align_matrices = generate_rotation_matrices(
        torch.tensor([-90.0, 90.0]),
        [0, 1],
    )
    pos = apply_rotations(pos, align_matrices)
    for i in range(frame // frame_per_motion):
        # print((i + 1) * frame_per_motion - 1)
        pos = apply_transformation(pos, root_motion[(i + 1) * frame_per_motion - 1])
    pos = apply_transformation(pos, root_motion[frame])
    gaussians._xyz = pos
...
def apply_grid_bc_w_freeze_pts(grid_size, grid_lim, freeze_pts, mpm_solver):
    ...
    # print("grid_lim, grid_size: ", grid_lim, grid_size)
    # print("dx, inv_dx: ", dx, inv_dx)
...
@torch.no_grad()
def grid_sample(pos, size, start, return_p2v=True):
    # pos: float [N, 3]
    # batch: long [N]
    # size: float [3, ]
    # start: float [3, ] / None
    ...
    cluster_ori = voxel_grid(pos, size)  # [N, ]

    unique, cluster, counts = torch.unique(cluster_ori, sorted=True, return_inverse=True, return_counts=True)
    print("Total grid num: ", unique.shape)
    ...
    return unique, cluster_ori, p2v_map
